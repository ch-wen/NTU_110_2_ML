...
class Classifier(nn.Module):
	def __init__(self, d_model=220, n_spks=600, dropout=0.1): #220
		super().__init__()
		...
		self.attention = SelfAttentionPooling(input_dim = d_model)
		#self.amsoft = AM_Softmax_v1()
		...
		#self.softmax = AM_Softmax_v1(m=0.03, s=30, d=d_model, num_classes=n_spks)
		#self.softmax = AMSoftmax(n_spks, n_spks, 0.1, 20)
		self.softmax = AdMSoftmaxLoss(
                                embedding_dim = n_spks, 
                                no_classes = n_spks, 
                                scale = 10.0, 
                                margin=0.1)
	...
	def forward(self, mels, labels):
		...
		out = self.pred_layer(stats)
		loss, logit = self.softmax(out, labels)
		return logit, loss
	...
	save_steps,
   	...
   	for step in range(total_steps):
   		...
   		if (step + 1) % save_steps == 0 and best_state_dict is not None:
   			torch.save(best_state_dict, save_path) ##redo
...
def main(
	...
	for feat_paths, mels in tqdm(dataloader):
		with torch.no_grad():
			mels = mels.to(device)
			labels = torch.zeros([len(mels)]).to(device)
			outs, l = model(mels, labels.long())
